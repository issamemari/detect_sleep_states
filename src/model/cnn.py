...
class OneDObjectDetectionCNN(nn.Module):
    def __init__(
        ...
        self.bbox_pred = nn.Conv1d(64, self.num_anchors * 2, kernel_size=1)  # Each anchor has 2 coordinates (start, end)
    ...
    def forward(self, x):
        ...
        scores = scores.view(scores.size(0), scores.size(1), self.num_anchors, self.num_classes)

        bboxes = bboxes.permute(0, 2, 1).contiguous()
        bboxes = bboxes.view(bboxes.size(0), bboxes.size(1), self.num_anchors, 2)
...
def main():
    ...
    # Example input (batch size of 1, 2 channel, signal length of 256)
    input_signal = torch.randn(1, 2, signal_length)
